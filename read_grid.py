...
def _write_grid_info(lons_row, lons_cor, lats_row, lats_cor, nlon, nlat,
    ...
    lt_row = np.array_split(lats_row, np.ceil(lats_row.size/6).astype(int)) 	#np.int
    lt_row_str = "\n".join([" ".join(str(item) for item in arr)
                            for arr in lt_row])
    lt_cor = np.array_split(lats_cor, np.ceil(lats_cor.size/6).astype(int)) 	#np.int
    lt_cor_str = "\n".join([" ".join(str(item) for item in arr)
                            for arr in lt_cor])
    ln_row = np.array_split(lons_row, np.ceil(lons_row.size/6).astype(int)) 	#np.int
    ln_row_str = "\n".join([" ".join(str(item) for item in arr)
                            for arr in ln_row])
    ln_cor = np.array_split(lons_cor, np.ceil(lons_cor.size/6).astype(int)) 	#np.int
    ln_cor_str = "\n".join([" ".join(str(item) for item in arr)
                            for arr in ln_cor])
